...
const sendJSON = (response) => (result) => response.json(result);
const sendError = (response) => (code, error) => response.status(code).json(error);
...
const promiseResove = async (promise, resove) => {
  try {
    const data = await promise;
    if (isEmpty(data)) resove.error(404, { message: 'resource not found.' });
    else resove.success(data);
  } catch(error) {
